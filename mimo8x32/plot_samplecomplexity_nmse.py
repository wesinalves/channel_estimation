...
def read_file(path, number, city, pretrained=0):
    if pretrained == 0:
        filename = os.path.join(path, f'mean_nmse_matched_Nr8_Nt32_numEx256_{city}_{number}.txt')
    else:
        filename = os.path.join(path, f'mean_nmse_pretrained_Nr8_Nt32_numEx256_{city}_{number}.txt')
    return filename
...
plt.plot(x, matched1, "r--s", label="1 bit casado")
plt.plot(x, proposed3, "k-P", label="3 bits DTL")
plt.plot(x, matched3, "r--P", label="3 bits casado")
plt.plot(x, proposed5, "k-o", label="5 bits DTL")
plt.plot(x, matched5, "r--o", label="5 bits casado")
