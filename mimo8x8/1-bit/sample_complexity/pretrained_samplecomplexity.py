...
model_filepath = f"models/{quantization}/model-rosslyn-conv.h5"
...
def save_bottleneck_features(input_train, input_val, baseline, instance, iteration):
    ...
    # save features
    with open(f'models/{quantization}/sample_complexity/bottleneck_features_train{instance}_iter{iteration}.npy', 'wb') as file1:
        np.save(file1, predicted_output)

    predicted_val = bottleneck_model.predict(input_val)
    with open(f'models/{quantization}/sample_complexity/bottleneck_features_val{instance}_iter{iteration}.npy', 'wb') as file2:
...
def train_top_model(output_train, output_val, instance, iteration):
    ...
    with open(f'models/{quantization}/sample_complexity/bottleneck_features_train{instance}_iter{iteration}.npy', 'rb') as file1:
        input_train = np.load(file1)
    with open(f'models/{quantization}/sample_complexity/bottleneck_features_val{instance}_iter{iteration}.npy', 'rb') as file2:
    ...
    top_model.save_weights(f'models/{quantization}/sample_complexity/bottleneck_fc_model_weights{instance}_iter{iteration}.h5')
...
for i in range(len(num_training_examples)):
    ...
    for it in range(5):
        ...
        bot_model = save_bottleneck_features(x_train, x_val, model, num_training_examples[i], it)
        t_model = train_top_model(y_train, y_val, num_training_examples[i], it)
        nmse_avg, nmse_min, nmse_max = test_model(bot_model, t_model, num_test_examples)
        all_nmse_avg.append(nmse_avg)
        all_nmse_min.append(nmse_min)
        all_nmse_max.append(nmse_max)

        del x_train, y_train, x_val, y_val
        del bot_model, t_model
    ...
    output_filename = (
        f"mean_nmse_pretrained_Nr{Nr}_Nt{Nt}_numEx{numSamplesPerExample}_rosslyn_china_{i}.txt"
    )
    ...
    del all_nmse_avg, all_nmse_max, all_nmse_min
    del nmse_avg, nmse_max, nmse_min

    
    

with open(os.path.join(logdir, "complexity10-100_proposed.txt"), 'w') as f:
