...
def save_bottleneck_features(input_train, input_val, baseline, instance):
    ...
    # save features
    with open(f'models/{quantization}/sample_complexity/bottleneck_features_train{instance}.npy', 'wb') as file1:
        np.save(file1, predicted_output)

    predicted_val = bottleneck_model.predict(input_val)
    with open(f'models/{quantization}/sample_complexity/bottleneck_features_val{instance}.npy', 'wb') as file2:
...
def train_top_model(output_train, output_val, instance):
    ...
    with open(f'models/{quantization}/sample_complexity/bottleneck_features_train{instance}.npy', 'rb') as file1:
        input_train = np.load(file1)
    with open(f'models/{quantization}/sample_complexity/bottleneck_features_val{instance}.npy', 'rb') as file2:
    ...
    top_model.save_weights(f'models/{quantization}/sample_complexity/bottleneck_fc_model_weights{instance}.h5')
