...
logdir = "./mimo8x8/pretrained/sample_complexity"
# save this script
...
def save_bottleneck_features(input_train, input_val, baseline, instance):
    ...
    # save features
    with open('models/pretrained/sample_complexity/bottleneck_features_train{}.npy'.format(instance), 'wb') as file1:
        np.save(file1, predicted_output)

    predicted_val = bottleneck_model.predict(input_val)
    with open('models/pretrained/sample_complexity/bottleneck_features_val{}.npy'.format(instance), 'wb') as file2:
...
def train_top_model(output_train, output_val, instance):
    ...
    with open('models/pretrained/sample_complexity/bottleneck_features_train{}.npy'.format(instance), 'rb') as file1:
        input_train = np.load(file1)
    with open('models/pretrained/sample_complexity/bottleneck_features_val{}.npy'.format(instance), 'rb') as file2:
    ...
    top_model.save_weights('models/pretrained/sample_complexity/bottleneck_fc_model_weights{}.h5'.format(instance))

    return top_model
  
def test_model(bottleneck_model, top_model, num_test_example, instance):
    ...
    output_filename = (
        f"all_nmse_pretrained_Nr{Nr}_Nt{Nt}_numEx{numSamplesPerExample}_rosslyn_china_{instance}.txt"
    )
...
for i in range(len(num_training_examples)):
    ...
    test_model(bot_model, t_model, num_test_examples, num_training_examples[i])

with open(os.path.join(logdir, "complexity10-100.txt"), 'w') as f:
