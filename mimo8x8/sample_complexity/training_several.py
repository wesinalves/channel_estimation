...
logdir = "./mimo8x8/sample_complexity"
# save this script
...
def train_model(input_train, output_train, input_val, output_val, instance):
    """train model"""
    ...
    matched_model.save_weights('models/sample_complexity/matched_model_weights{}.h5'.format(instance))

    return matched_model

def test_model(my_model, num_test_example, instance):
    ...
    output_filename = (
        f"all_nmse_Nr{Nr}_Nt{Nt}_numEx{numSamplesPerExample}_china_{instance}.txt"
    )
...
for number in range(len(num_training_examples)):
    ...
    model = train_model(y_train, h_train, y_val, h_val, num_training_examples[number])
    test_model(model, num_test_examples, num_training_examples[number])
